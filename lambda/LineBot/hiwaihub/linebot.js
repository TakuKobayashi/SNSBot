...
var LineBot = function (accessToken) {
  ...
  this.searchPornhub = async function (keyword) {
    var searcher = new Pornsearch(keyword);
    return await searcher.videos();
  }

  this.checkConfirmed = async function (user_id) {
  ...
  this.updateConfirmState = async function (user_id, timestamp) {
    const finsByUserObj = {
      user_id: user_id
    }
    const userData = await dynamodb.getPromise("users", finsByUserObj);
